...
def updateStaff():
    ...
    """
    datacenter.pushdata(lenh.format(staffToEdit.getFullName(), staffToEdit.getPhoneNumber(
    ), staffToEdit.getAddress(), staffToEdit.getSalary(), staffToEdit.getPosition(), staffToEdit.getID()))
...
def addStaff():
    ...
    """
    datacenter.pushdata(lenh.format(listStaff.getLen() + 1, s.getFullName(),
                        s.getPhoneNumber(), s.getAddress(), s.getSalary(), "*", s.getPosition()))
...
@rootflaskapp.app.route("/checkExist", methods=["POST"])
def checkExistRoute():
    data = rootflaskapp.request.get_json()
    if checkExist(data["Id"]) == True:
        return {"status": "true"}
    return {"status": "false"}
