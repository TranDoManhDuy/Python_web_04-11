...
@rootflaskapp.app.route("/checkreadyVehicle", methods=["POST"])
def checkreadyVehicle():
    if rootflaskapp.request.method == "POST":
        data = rootflaskapp.request.get_json()
        if danhsachPT.layPttheoSDK(data["id"])["ready"] == "SANSANG":
...
def products_addPT():
    if rootflaskapp.request.method == "POST":
        ...
        query = (
            ...
            f"VALUES ('{pt.getId()}', '{pt.getDanhmuc()}', '{
                pt.getLoaiphuongtien()}', "
            f"'{pt.getSodangki()}', '{pt.getTenphuongtien()}', {
                pt.getSochongoi()}, "
...
def fixVehicle():
    if rootflaskapp.request.method == "POST":
        ...
        execute = """
            ...
            WHERE ID = '{}'
        """
        datacenter.pushdata(execute.format(data["danhmuc"], data["loaiphuongtien"], data["sodangki"],
                            data["tenphuongtien"], data["sochongoi"], data["giathue1n"], data["tinhtrangxe"], int(data["id"])))
        fetchDataFromDB()
...
def downloadFileFunc():
    ...
    file_path = os.path.join(
        rootflaskapp.app.config['UPLOAD_FOLDER'], 'dataProductslist.xlsx')
